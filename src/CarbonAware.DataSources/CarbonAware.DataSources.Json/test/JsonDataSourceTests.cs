...
public class JsonDataSourceTests
{
    ...
    [Test]
    public async Task GetCurrentCarbonIntensityForecastAsync_ByLocation()
    {
        var mockDataSource = SetupMockDataSource();
        ...
        Assert.AreEqual("westus", result!.Location.Name);
        Assert.AreEqual(2, result.ForecastData.Count());
    }

    [Test]
    public async Task GetCurrentCarbonIntensityForecastAsync_NoMatchedCriteria()
    {
        var mockDataSource = SetupMockDataSource();

        var location = new Location { Name = "kyiv" };
    ...
    private Mock<JsonDataSource> SetupMockDataSource()
    {
        ...
        mockDataSource.Protected()
            ...
            .Verifiable();
        mockDataSource.Protected()
            .Setup<Task<List<EmissionsForecast>?>>("GetJsonEmissionsForecastsAsync")
            .ReturnsAsync(GetTestEmissionsForecasts)
            .Verifiable();
    ...
    private List<EmissionsForecast> GetTestEmissionsForecasts()
    {
        // All the tests above correspond to values in this mock data. If the mock values are changed, the tests need to be updated 
        return new List<EmissionsForecast>() {
                new EmissionsForecast {
                    Location = new Location { Name = "eastus" },
                ...
                new EmissionsForecast {
                    Location = new Location { Name = "westus" },
                    ForecastData = new List<EmissionsData> {
                        new EmissionsData {
                            Location = "westus",
                            Rating = 100,
