...
COPY ./src/dotnet/ ./
RUN dotnet publish  ./src/dotnet/CarbonAware.CLI/CarbonAware.CLI.csproj -c Release -o out --no-self-contained
RUN cp ./src/dotnet/CarbonAware.CLI/carbon-aware.json out
RUN cp -r  ./src/dotnet/data/data-files/ out
